...
for i in range(len(dataset_list)):
    ...
    # df = import_extra_dataset(params)
    ...
    if not os.path.exists(file_path):
        os.mkdir(file_path)
    save_file_path = os.path.join(file_path, '{}_{}_{}'.format(params['Dataset'], 'IFCGAT', time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
    # save_file_path = os.path.join(file_path, '{}_{}_{}'.format(params['Dataset'], 'AIFC',
    ...
    for j in range(0, 50):
        ...
        train_loader = DataLoader(train_set, collate_fn=collate_fraggraphs, batch_size=len(train_set), shuffle=True)
        ...
        #                                path='checkpoint_seed' + params['Dataset'] + 'AIFC' + '.pt')
