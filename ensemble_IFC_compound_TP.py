# -*- coding: utf-8 -*-
# @Time    : 2025/6/22 09:48
# @Author  : kai kai
# @Site    : 
# @File    : ensemble_IFC_compound_TP.py
# @Software: PyCharm
...
from utils.piplines_TP import evaluate_IFC_TP,  evaluate_IFC_descriptors_TP, evaluate_IFC_attention_TP
...
for j in range(len(splitting_seed)):
    ...
    for i in range(100):
        ...
        _, _, train_predict, train_target, train_smiles = evaluate_IFC_TP(model, scaling, train_loader, n_param, flag=True)
        _, _, val_predict, val_target, val_smiles = evaluate_IFC_TP(model, scaling, val_loader, n_param, flag=True)
        _, _, test_predict, test_target, test_smiles = evaluate_IFC_TP(model, scaling, test_loader, n_param, flag=True)
        if i == 0:
    ...
    all_smiles, all_descriptors = evaluate_IFC_descriptors_TP(model, scaling, raw_loader, n_param)

    df_descriptors = pd.DataFrame(all_descriptors.detach().to(device='cpu').numpy())
    df_descriptors['SMILES'] = all_smiles

    df_merge = pd.merge(df_results, df_descriptors, how='outer', on='SMILES')

    save_file_path = os.path.join('./library/' + path, '{}_{}_{}_{}_{}'.format(name, 'viscosity_descriptors', seed, 'OP' + str(op_idx), time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
    df_merge.to_csv(save_file_path, index=False)
    all_predictions, all_attention = evaluate_IFC_attention_TP(model, scaling, raw_loader)
    ...
    df_attention_path = os.path.join('./library/' + path, '{}_{}_{}_{}_{}'.format(name, 'viscosity_attention', seed, 'OP' + str(op_idx), time.strftime('%Y-%m-%d-%H-%M')) + '.csv')
