...
def collate_fraggraphs(samples): # notice PT
    # origin_graphs, motif_graphs: list of graphs:
    # origin_graphs, frag_graphs, motif_graphs, _, targets, smiles, names, temperature, pressure = map(list, zip(*samples))
    origin_graphs, frag_graphs, motif_graphs, _, targets, smiles, names = map(list, zip(*samples))
    targets = torch.tensor(np.array(targets)).unsqueeze(1)
    # temperature = torch.tensor(np.array(temperature)).unsqueeze(1)
    # pressure = torch.tensor(np.array(pressure)).unsqueeze(1)
    batched_origin_graph = dgl.batch(origin_graphs)
    batched_frag_graph = dgl.batch(frag_graphs)
    batched_motif_graph = dgl.batch(motif_graphs)
    # return batched_origin_graph, batched_frag_graph, batched_motif_graph, targets, smiles, names, temperature, pressure
